...
- **회사 및 직무 정보 입력:** 지원 회사명, 직무명, 그리고 해당 채용 공고(Job Description, JD) 텍스트를 입력합니다.
- **포트폴리오/이력서 업로드:** PDF 형식의 이력서 및 포트폴리오를 업로드합니다.
- **백엔드 분석 파이프라인:**
...
- **화상 인터페이스:**
  - 화면 중앙: 사용자의 로컬 웹캠 화면 스트리밍 (MediaStream API).
  - 화면 사이드: 가상 면접관의 상태(말하는 중, 듣는 중, 분석 중)를 시각화하는 오디오 Waveform 애니메이션 및 면접 타이머.
- **비언어적 요소 실시간 분석 (Edge-side AI):**
  ...
  - 시선 방향(카메라 응시 여부), 머리 각도, 주요 표정(눈썹 찡그림, 미소 등) 데이터를 프레임당 추출하고, 이 메타데이터 포인트들만 로컬 상태에 저장하거나 백엔드로 가볍게 전송합니다.
- **실시간 대화 & 꼬리 질문 루프:**
  1. **질문 송출:** LLM이 생성한 텍스트 질문을 TTS(Text-to-Speech) API를 통해 고음질 오디오로 합성하여 브라우저에서 재생합니다.
  2. **답변 수집:** 사용자가 답변을 시작하면 마이크 입력을 녹음하고, Web Speech API 또는 Whisper API를 활용해 실시간으로 텍스트(STT)화합니다.
  3. **실시간 꼬리 질문 생성:** 답변이 완료되면(또는 사용자가 완료 버튼을 클릭하면) STT 변환 텍스트를 백엔드로 전달합니다. LLM은 답변의 구체성, 기술 스택의 깊이, 모순점 등을 실시간 분석하여 예리한 **꼬리 질문(Follow-up Question)**을 즉시 생성하여 질문 루프를 이어갑니다.

### 2.3 Step 3: 다차원 피드백 리포트 제공 (Report Phase)
- **논리 구조 분석 (STAR 기법):**
  - 사용자의 각 답변이 **STAR(Situation, Task, Action, Result)** 구조에 얼마나 적합한지 평가하고 점수화합니다.
  - 예: "행동(Action)은 구체적이나, 그에 따른 정량적 결과(Result)에 대한 설명이 부족합니다."와 같은 피드백 제공.
- **비언어적 분석 시각화:**
  - **시선 처리:** 전체 면접 시간 동안 카메라를 올바르게 응시한 비율(%)을 타임라인 차트로 시각화.
  - **감정 및 표정:** 긴장 수준(얼굴 떨림, 입술 긴장 등) 및 긍정적 표정(미소) 추이 제공.
  - **스피치 분석:** 분당 발화 수(WPM, Words Per Minute) 분석 및 "음", "어", "그" 등 불필요한 필러 워드(Filler Words) 감지 및 횟수 시각화.
- **AI 모범 답변 스크립트 제안:**
  - 사용자의 원래 답변 스크립트와 면접관의 질문 의도를 바탕으로, STAR 기법을 완벽히 적용해 다듬은 **AI 추천 모범 답변**을 대조하여 제공합니다.
...
- [ ] MediaPipe 로컬 오버헤드 최적화 (FPS 조절을 통한 CPU 점유율 하향 조정).
